...
function friendsOfRecursion(name, adjacencyList, visited, maxDistance, currentDistance = 0) {
  ...
  name.foreach(friend =>  {
    ...
    if (!visited.has.friend) {
      console.log(`visiting friend: ${friend}`)
      visited.add(
            friendsOfRecursion(
              friend,
              adjacencyList,
...
function friendsOf(adjacencyList, name, distance) {
  if (!adjacencyList.length) return undefined
  if (breadthFirstSearch(adjacencyList, name)) return undefined


  let visited = new Set()
  console.log(`check`)
  friendsOfRecursion(name, adjacencyList, visited, distance)
  return visited;
